...
int *STATUS 			= (int)(TIMER_1_0_BASE);
int *CONTROL 			= (int)(TIMER_1_0_BASE+4);
int *PERIOD_LOW 		= (int)(TIMER_1_0_BASE+8);
int *PERIOD_HIGH 		= (int)(TIMER_1_0_BASE+12);
int *SNAP_LOW 			= (int)(TIMER_1_0_BASE+16);
int *SNAP_HIGH 			= (int)(TIMER_1_0_BASE+20);
...
void measure(void* context){
	...
	*STATUS = 0x2;
	...
	TIMER = perf_get_section_time(PERFORMANCE_COUNTER_1_BASE, 1);
	...
	if(ITERATION == 1){
		...
		MPC_shared_memory_write(TIME_MIN, 10+CPU_1);
		MPC_shared_memory_write(TIME_MAX, 20+CPU_1);
...
int main()
{
	...
	*CONTROL = 0x7;
	...
	alt_ic_isr_register(TIMER_1_0_IRQ_INTERRUPT_CONTROLLER_ID, TIMER_1_0_IRQ, measure, NULL, NULL);

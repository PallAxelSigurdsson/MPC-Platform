...
int main()
{
	...
	MPC_initialize_fifos(CPU_7, 12, 1);
	MPC_receive_from_CPU(CPU_6, CPU_7);
...
/****************/
	...
	for(INDEX = 0; INDEX < ITERATIONS; INDEX++){
		...
		MPC_initialize_fifos(7,ALMOST_FULL,1);
	...
	for(INDEX = 0; INDEX < ITERATIONS; INDEX++){
		...
		MPC_initialize_fifos(7,12,ALMOST_EMPTY);
	...
	for(INDEX = 0; INDEX < ITERATIONS; INDEX++){
		...
		MPC_initialize_fifos(7,ALMOST_FULL,ALMOST_EMPTY);

 ...
 * for the IOWR_ALTERA_AVALON_MUTEX_MUTEX(MUTEXES.MUTEX_0->mutex_base, data) function.
 */
...
int *STATUS 			= (int)(TIMER_6_0_BASE);
int *CONTROL 			= (int)(TIMER_6_0_BASE+4);
int *PERIOD_LOW 		= (int)(TIMER_6_0_BASE+8);
int *PERIOD_HIGH 		= (int)(TIMER_6_0_BASE+12);
int *SNAP_LOW 			= (int)(TIMER_6_0_BASE+16);
int *SNAP_HIGH 			= (int)(TIMER_6_0_BASE+20);
...
void measure(void* context){
	...
	*STATUS = 0x2;
	...
	if(ITERATION == 10000){
		...
		MPC_shared_memory_write(TIME_MIN, 10+CPU_6);
		MPC_shared_memory_write(TIME_MAX, 20+CPU_6);

		printf("\033c");

		printf("\n\nCongestion test - IOWR_ALTERA_AVALON_MUTEX_MUTEX(MUTEXES.MUTEX_0->mutex_base, data) \n\n");
...
int main()
{
	...
	*CONTROL = 0x7;
	...
	alt_ic_isr_register(TIMER_6_0_IRQ_INTERRUPT_CONTROLLER_ID, TIMER_6_0_IRQ, measure, NULL, NULL);
